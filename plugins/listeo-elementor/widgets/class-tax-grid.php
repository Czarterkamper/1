...
class TaxonomyGrid extends Widget_Base {
	...
	public function get_title() {
		return __( 'Taxonomy Grid', 'elementor-listeo' );
	}
   ...
   //          'autoplayspeed'      => '3000',
		...
		foreach ($taxonomy_names as $key => $value) {
			...
			$this->add_control(
				$value->name.'_include',
				[
					'label' => __( 'Include listing from '.$value->label, 'elementor-listeo' ),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
			...
			$this->add_control(
				$value->name.'_exclude',
				[
					'label' => __( 'Exclude listings from '.$value->label, 'elementor-listeo' ),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
		...
		$this->add_control(
			'number',
			[
				'label' => __( 'Terms to display', 'elementor-listeo' ),
		...
		$this->add_control(
			'only_top',
			[
				'label' => __( 'Show only top terms', 'elementor-listeo' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'options' => [
					'default' => __( 'Default', 'elementor-listeo' ),
					'alt' => __( 'Alternative', 'elementor-listeo' ),
	...
	protected function render() {
		...
		<div class="categories-boxes-container<?php if($settings['style']=='alt'){ echo "-alt"; }?> margin-top-5 margin-bottom-30">
			...
			<a href="<?php echo get_term_link( $term ); ?>" class="category-small-box<?php if($settings['style']=='alt'){ echo "-alt"; }?>">
				...
				<?php if($settings['show_counter']=="yes") { ?><span  class="category-box-counter<?php if($settings['style']=='alt'){ echo "-alt"; }?>"><?php echo $term->count ?></span> <?php } ?>
