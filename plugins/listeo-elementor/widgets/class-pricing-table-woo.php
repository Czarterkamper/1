...
class PricingTableWoo extends Widget_Base {
	...
	public function get_title() {
		return __( 'Pricing Table WooCommerce', 'elementor-listeo' );
	}
 ...
 //        "buttontext"    => 'Sign Up',
		...
		$this->add_control(
			...
			[
				...
				'default' => 'date',
				'options' => [
					'price' 		=>  __( 'Price', 'elementor-listeo' ),
					'price-desc' 	=> __( 'Price desc', 'elementor-listeo' ),
					'rating' 		=> __( 'Rating', 'elementor-listeo' ),
					'title' 		=> __( 'Title', 'elementor-listeo' ),
					'popularity' 	=> __( 'Popularity', 'elementor-listeo' ),
					'random' 		=> __( 'Random', 'elementor-listeo' ),
		...
		$this->add_control(
			'columns_per_row',
			[
				'label' => __( 'Columns in a row', 'elementor-listeo' ),
		...
		$this->add_control(
			'buttonlink',
			[
				'label' => __( 'Option URL for add Listing overide','elementor-listeo' ),
            ...
            <div class="plan <?php echo ($product->is_featured()) ? 'featured' : '' ; ?>">
                <?php if( $product->is_featured() ) : ?>
                    <div class="listing-badge">
                        <span class="featured"><?php esc_html_e('Featured','elementor-listeo') ?></span>
                    </div>
                <?php endif; ?>
                ...
                <div class="plan-features">
                    <ul class="plan-features-auto-wc">
                        ...
                        if(!$listingslimit){
                            echo "<li>";
                             esc_html_e('Unlimited number of listings','elementor-listeo'); 
                             echo "</li>";
                        } else { ?>
                            <li>
                                <?php esc_html_e('This plan includes ','elementor-listeo'); printf( _n( '%d listing', '%s listings', $listingslimit, 'elementor-listeo' ) . ' ', $listingslimit ); ?>
                            </li>
                        <?php } ?>
                        <li>
                            <?php esc_html_e('Listings are visible ','elementor-listeo'); printf( _n( 'for %s day', 'for %s days', $product->get_duration(), 'elementor-listeo' ), $product->get_duration() ); ?>
                        </li>
                    ...
                    <?php 
                        ...
                        $label  = apply_filters( 'add_to_cart_text', esc_html__( 'Add Listing', 'elementor-listeo' ) );
